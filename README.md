...
- **Provider Credit Monitor**:
  ...
  - The cron service's schedule lives in `railway.cron.json`, not in the Railway dashboard. A schedule set only through the dashboard/API is attached to the deployment that existed when it was set — a later `git push` builds a deployment without it and silently disarms the cron. Pointing the service at a config file in the repo (Settings → Config as code) makes every deploy carry it. The API service must keep its config-file field empty, or it would inherit the cron schedule and stop serving.
