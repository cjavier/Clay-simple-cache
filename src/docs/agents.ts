...
GET  /credits -> {status:"green"|"yellow"|"red",checked_at,burn_per_day:{searches,verifications},providers:[{provider,label,status,balance,unit,days_left,error,impact}]} (live probe of every paid provider's balance)
GET  /credits/history?provider?&limit?=100 -> {count,history:[{provider,status,balance,unit,days_left,error,checked_at}]}
...
check_credits {} -> live green/yellow/red balance check of every paid provider (EmailListVerify, DeBounce, Serper, DeepSeek). Call this first when find_email/verify_email keep returning status "unknown": a depleted provider looks exactly like "not found".`;

/** Operating rules shared by REST and MCP consumers. */
export const AGENT_RULES = `Rules for agents (REST and MCP alike): (1) always authenticate — REST via header "Authorization: Bearer <API_KEY>", MCP via the same Bearer header on the /mcp connection; (2) before contacting a lead, check that client's Do Not Contact list first — REST: GET /profiles, GET /companies, POST /find, or POST /verify with dnc_client=<handle>, or POST /dnc/check directly; MCP: call dnc_check (or pass dnc_client to find_email/verify_email/get_profile/get_company) — and treat any {"do_not_contact":true,...} result as a hard stop. Do not retry without the DNC gate to "get around" a block; (3) find_email/verify_email (REST: /find, /verify) and generate_copy/explore (REST: /copy, /explore) cost real money or third-party API usage per call — prefer get_profile/get_company (REST: GET /profiles, GET /companies) cache lookups first, and check get_stats (REST: GET /stats) for running totals; (4) all POST bodies/tool inputs are JSON, max 1mb on the REST side; (5) back off and retry with delay on HTTP 429 (rate limited); (6) if find_email/verify_email return status "unknown" repeatedly, call check_credits (REST: GET /credits) before concluding the emails do not exist — a provider with no balance returns "unknown", not an error.`;
